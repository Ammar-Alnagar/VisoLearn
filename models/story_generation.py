...
def generate_initial_story(topic_focus, difficulty, age, autism_level):
    ...
    query = f"""
    You are a professional screenwriter crafting an initial draft of a story for autistic learners (DO NOT include minor characters).
    If the concept involves a child, age them up to an adult. Design a cohesive, logically-sequenced story about '{topic_focus}' with exactly {num_scenes} scenes.
...
def critique_story(story_data, topic_focus, difficulty, age, autism_level):
    ...
    """
    query = f"""
    You are a professional story editor reviewing a story for autistic learners (no minor characters).
...
def generate_story_with_custom_description(topic_focus, user_description, difficulty, age, autism_level):
    ...
    query = f"""
    You are a professional storyteller and screenwriter designing an engaging, cinematic story for autistic learners (no minor characters). If any character is a child, age them up to an adult.
