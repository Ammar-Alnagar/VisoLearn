...
class ComicImageGenerator:
    ...
    def _get_attempt_specific_prompt(self, base_prompt, num_scenes, attempt):
        ...
        strategies = [
            f"EXACTLY {num_scenes} panels with bold borders",
    ...
    def _create_mathematical_perfect_comic(self, title, description, num_scenes, style):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                y1 = panel_area_y + row * panel_height
                x2 = x1 + panel_width - 5
    ...
    def split_comic_into_scenes(self, comic_image, num_scenes, preferred_layout=None, use_gemini_analysis=True):
        ...
        if preferred_layout:
            ...
            # Always use Gemini Vision analysis - trust what it sees
            if use_gemini_analysis:
                ...
                print("⚠️ Gemini analysis disabled. Using simple square grid as fallback.")
                # Simple fallback only when Gemini is explicitly disabled
                import math
                cols = math.ceil(math.sqrt(num_scenes))
                rows = math.ceil(num_scenes / cols)
        ...
        if actual_panels == num_scenes:
            print(f"✅ Using {rows}×{cols} grid layout - matches expected {num_scenes} scenes")
        else:
            print(f"🎯 Using {rows}×{cols} grid layout - detected {actual_panels} panels (expected {num_scenes})")

        # Calculate scene dimensions
        scene_width = width // cols
        scene_height = height // rows

        # Generate grid coordinates - ensure we get exactly num_scenes panels
        grid = []
        panels_created = 0

        for row in range(rows):
            for col in range(cols):
                if panels_created < num_scenes:  # Only create requested number of panels
                    x1 = col * scene_width
                    y1 = row * scene_height
                    x2 = (col + 1) * scene_width
                    y2 = (row + 1) * scene_height
    ...
    def _calculate_optimal_layout(self, num_scenes, image_width, image_height):
        ...
        for rows, cols in valid_layouts:
            ...
            panel_aspect_ratio = (image_width / cols) / (image_height / rows)
            ...
            total_score = aspect_ratio_diff + panel_aspect_penalty + square_preference
